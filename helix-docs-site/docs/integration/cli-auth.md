...
## The helix CLI (worked example)

The `helix` CLI is a native-app client. Its `login` command is **mode-aware**.
...
=== "Browser (default)"
    ...
    ```bash
    HELIX_NO_BROWSER=1 helix login
    ```
...
```bash
helix whoami     # who am I, which realm, token status
helix logout     # remove the stored session
```

The session is stored under `~/.helix/config.yaml` (override with `HELIX_CONFIG`); the auth server is `HELIX_IDP_URL`.

## Enable it for your own native app

The capability is not specific to the helix CLI — **any** public client you register can use it. Two steps:
...
That's it. Your app now gets browser + device login and a rotating, long-lived session, just like `helix`.
...
## Environment variables (helix CLI)

| Variable | Purpose |
| --- | --- |
| `HELIX_IDP_URL` | Auth-server base URL (realm paths are appended) |
| `HELIX_CONFIG` | Path to the CLI config (default `~/.helix/config.yaml`) |
| `HELIX_NO_BROWSER` | Any value ⇒ print the sign-in URL instead of launching a browser |
