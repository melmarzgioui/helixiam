-- Helix IAM PROD-4 — Flyway baseline (V1). The canonical IAM schema, captured as the first
-- versioned migration. IDENTICAL to schema.sql (the legacy spring.sql.init bootstrap) EXCEPT the
-- single `user_in_role` table is relocated to the end: schema.sql relies on a tolerant two-pass
-- apply for that one forward reference (it FKs tenant_user, declared just after it), but Flyway
-- applies a migration in ONE strict pass. Moving that one block last makes V1 single-pass valid
-- while every other statement stays verbatim. All statements are idempotent (IF NOT EXISTS), so
-- applying V1 over an already-provisioned database is a harmless no-op. NEVER edit this file after
-- release (Flyway checksums it); add forward changes as new V2__*, V3__* files. See DEPLOY.md.
...
-- (relocated from its original position for single-pass ordering — see header)
