package group.mfnr.authorization.amqp.gdpr.adapter;

import com.fasterxml.jackson.core.type.TypeReference;
import group.mfnr.authorization.amqp.gdpr.GdprConsentRecordDto;
import group.mfnr.authorization.amqp.gdpr.GdprConsentWithdrawDto;
import group.mfnr.authorization.amqp.gdpr.GdprConsentWriteDto;
import group.mfnr.authorization.amqp.gdpr.GdprEraseDto;
import group.mfnr.authorization.amqp.gdpr.GdprEraseResultDto;
import group.mfnr.authorization.amqp.gdpr.GdprExportDto;
import group.mfnr.authorization.amqp.gdpr.GdprPublisher;
import group.mfnr.authorization.amqp.gdpr.GdprUserRef;
import group.mfnr.authorization.amqp.support.DtoBridge;
import group.mfnr.authorization.service.gdpr.ConsentLedgerService;
import group.mfnr.authorization.service.gdpr.GdprAdminService;
...
public class GdprLocalAdapter implements GdprPublisher {
    ...
    public GdprEraseResultDto erase(final GdprEraseDto erase) {
        return bridge.to(gdpr.erase(
                bridge.to(erase, group.mfnr.authorization.domain.gdpr.GdprEraseDto.class)),
    ...
    public GdprConsentRecordDto recordConsent(final GdprConsentWriteDto write) {
        return bridge.to(consents.record(
                bridge.to(write, group.mfnr.authorization.domain.gdpr.GdprConsentWriteDto.class)),
    ...
    public Boolean withdrawConsent(final GdprConsentWithdrawDto withdraw) {
        return consents.withdraw(
                bridge.to(withdraw, group.mfnr.authorization.domain.gdpr.GdprConsentWithdrawDto.class));
    }
}
