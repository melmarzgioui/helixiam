...
import group.mfnr.authorization.repository.realm.RealmKeyRepository;
...
public class RealmKeyService {
     ...
     */
    public List<group.mfnr.authorization.domain.realm.RealmKeyView> listViews(final String realmId) {
        return realmKeyRepository.findAllByRealmIdOrderByCreationDateDesc(realmId).stream()
                .map(group.mfnr.authorization.domain.realm.RealmKeyView::from)
                .toList();
    }
