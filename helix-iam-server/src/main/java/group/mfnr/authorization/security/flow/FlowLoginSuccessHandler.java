...
import group.mfnr.authorization.flow.BrowserFlows;
import group.mfnr.authorization.flow.FlowExecutionState;
import group.mfnr.authorization.flow.FlowExecutor;
import group.mfnr.authorization.flow.FlowProgress;
...
public class FlowLoginSuccessHandler implements AuthenticationSuccessHandler {
    ...
    private final group.mfnr.authorization.security.realm.SessionPolicyApplier sessionPolicyApplier;
    // Auth-hardening (feature 6): optional per-realm concurrent-session cap, applied on completion.
    private final group.mfnr.authorization.security.realm.ConcurrentSessionLimiter concurrentSessionLimiter;
    private final group.mfnr.authorization.security.session.AuthTimeStamper authTimeStamper =
            new group.mfnr.authorization.security.session.AuthTimeStamper();
    ...
    public FlowLoginSuccessHandler(final FlowExecutor flowExecutor, final AuthFlowPublisher authFlowPublisher,
                                   ...
                                   final group.mfnr.authorization.security.realm.SessionPolicyApplier sessionPolicyApplier) {
    ...
    public FlowLoginSuccessHandler(final FlowExecutor flowExecutor, final AuthFlowPublisher authFlowPublisher,
                                   final AuthFlowMapper authFlowMapper, final String spBaseUrl,
                                   final group.mfnr.authorization.security.realm.SessionPolicyApplier sessionPolicyApplier,
                                   final group.mfnr.authorization.security.realm.ConcurrentSessionLimiter concurrentSessionLimiter) {
        ...
        switch (progress.type()) {
            case COMPLETED -> {
                ...
                if (concurrentSessionLimiter != null) {
                    ...
                    final group.mfnr.authorization.security.realm.ConcurrentSessionLimiter.Outcome outcome =
                            concurrentSessionLimiter.enforceOnLogin(RealmContextHolder.get(), authentication.getName(), sessionId);
                    if (outcome == group.mfnr.authorization.security.realm.ConcurrentSessionLimiter.Outcome.DENIED) {
