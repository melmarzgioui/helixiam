...
import group.mfnr.authorization.amqp.group.GroupRef;
import group.mfnr.authorization.amqp.group.GroupWriteDto;
...
public class RealmImportService {
    ...
    private void importAgents(final String realmId, final RealmExportDocument doc, final ImportOptions opts,
        ...
        final Map<String, group.mfnr.authorization.amqp.agent.AgentIdentityDto> existing =
                byKey(agentPublisher.list(realmId), group.mfnr.authorization.amqp.agent.AgentIdentityDto::name);
        for (final group.mfnr.authorization.amqp.agent.AgentIdentityDto a : doc.agents()) {
            if (a == null || isBlank(a.name())) {
                r.skipped(SLICE_AGENTS);
                continue;
            }
            final group.mfnr.authorization.amqp.agent.AgentIdentityDto current = existing.get(a.name());
            if (blocked(current != null, SLICE_AGENTS, a.name(), opts, r)) {
                continue;
            }
            try {
                agentPublisher.save(new group.mfnr.authorization.amqp.agent.AgentIdentityDto(
            ...
            for (final ServiceAccountRoleDto sa : doc.serviceAccountRoles()) {
                ...
                final Map<String, ServiceAccountRoleDto> existing = byKey(
                        clientRolePublisher.serviceAccountRoles(
                                group.mfnr.authorization.support.RealmScopedKey.pack(realmId, sa.clientId())),
    ...
    private void importResourceIndicators(final String realmId, final RealmExportDocument doc, final ImportOptions opts,
        ...
        for (final AllowedResourcesWrite aw : doc.resourceIndicators()) {
            ...
            final boolean exists = !nonNull(resourcePublisher.allowedResourcesForClient(
                    group.mfnr.authorization.support.RealmScopedKey.pack(realmId, aw.clientId()))).isEmpty();
