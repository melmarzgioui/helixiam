    ...
    public TestResult test(@PathVariable final String realmId, @PathVariable final String channel,
        ...
        try {
            ...
            if ("PUSH".equalsIgnoreCase(channel)) {
                ...
                final List<group.mfnr.authorization.amqp.messaging.DevicePushTokenDto> tokens = List.of(
                        new group.mfnr.authorization.amqp.messaging.DevicePushTokenDto(realmId, "test-user", "FCM", request.to()),
                        new group.mfnr.authorization.amqp.messaging.DevicePushTokenDto(realmId, "test-user", "APNS", request.to()));
