...
import group.mfnr.authorization.session.logout.FrontchannelLogoutRenderer;
import group.mfnr.authorization.session.logout.LogoutTargetResolver;
...
public class SsoLogoutResponseHandler implements AuthenticationSuccessHandler {
    ...
    private final group.mfnr.authorization.federation.FederatedLogoutCoordinator federatedLogoutCoordinator;
    ...
    public SsoLogoutResponseHandler(final SsoLogoutService ssoLogoutService, final LogoutTargetResolver targetResolver,
                                    ...
                                    final group.mfnr.authorization.federation.FederatedLogoutCoordinator federatedLogoutCoordinator) {
    ...
    public void onAuthenticationSuccess(final HttpServletRequest request, final HttpServletResponse response,
        ...
        if (authentication instanceof OidcLogoutAuthenticationToken token) {
            ...
            if (subject != null) {
                ...
                realm = group.mfnr.authorization.security.realm.RealmContextHolder.get();
        ...
        if (session != null) {
            final Object idpAlias = session.getAttribute(
                    group.mfnr.authorization.security.flow.FederationBrokerController.IDP_SOURCE_ATTR);
            if (idpAlias instanceof String alias && !alias.isBlank()) {
                final Object idpSubject = session.getAttribute(
                        group.mfnr.authorization.security.flow.FederationBrokerController.IDP_SUBJECT_ATTR);
                federatedLogoutCoordinator.propagate(new group.mfnr.authorization.federation.spi.IdentityProvider
