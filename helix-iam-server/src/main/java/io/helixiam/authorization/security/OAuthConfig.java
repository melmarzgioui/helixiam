...
import io.helixiam.authorization.security.mapper.ProtocolMapperResolver;
...
public class OAuthConfig {
    ...
    public org.springframework.security.oauth2.server.authorization.token.OAuth2TokenGenerator<? extends org.springframework.security.oauth2.core.OAuth2Token> tokenGenerator(
        ...
        final io.helixiam.authorization.security.device.NativeAppRefreshTokenGenerator refreshTokenGenerator =
                new io.helixiam.authorization.security.device.NativeAppRefreshTokenGenerator();
     ...
     */
    @Bean
    public io.helixiam.authorization.security.fapi.ClientCertificateResolver clientCertificateResolver(
            @org.springframework.beans.factory.annotation.Value("${helix.fapi.client-cert-header:X-Client-Cert}") final String header) {
        return new io.helixiam.authorization.security.fapi.ClientCertificateResolver(header);
    }
    ...
    public OAuth2TokenCustomizer<JwtEncodingContext> jwtTokenCustomizer(final UserInfoService userInfoService,
                                                                        ...
                                                                        final io.helixiam.authorization.amqp.org.OrganizationAdminPublisher organizationAdminPublisher,
        ...
        return context -> {
            ...
            io.helixiam.authorization.security.resource.ResourceIndicatorTokenCustomizer
    ...
    private static void applyOrganizations(final JwtEncodingContext context,
                                           final io.helixiam.authorization.amqp.org.OrganizationAdminPublisher organizationAdminPublisher) {
        try {
            final java.util.List<io.helixiam.authorization.amqp.org.OrgMembershipDto> memberships =
            ...
            for (final io.helixiam.authorization.amqp.org.OrgMembershipDto m : memberships) {
    ...
    private static void applyCertificateBinding(final JwtEncodingContext context,
                                                final io.helixiam.authorization.security.fapi.ClientCertificateResolver resolver) {
        try {
            ...
            resolver.resolve(attrs.getRequest()).ifPresent(cert -> {
                final String x5t = io.helixiam.authorization.security.fapi.CertificateThumbprint.x5tS256(cert);
                context.getClaims().claims(claims -> claims.put("cnf",
                        io.helixiam.authorization.security.fapi.MtlsConfirmation.mergeX5t(claims.get("cnf"), x5t)));
            });
        } catch (final Exception e) {
    ...
    private static ResolvedRoles resolveRoles(final JwtEncodingContext context, final UserPublisher userPublisher,
        ...
        if (context.getAuthorizationGrantType() == AuthorizationGrantType.CLIENT_CREDENTIALS) {
            try {
                final List<io.helixiam.authorization.amqp.clientrole.ServiceAccountRoleDto> saRoles =
                        clientRolePublisher.serviceAccountRoles(io.helixiam.authorization.support.RealmScopedKey.pack(
                                io.helixiam.authorization.security.realm.RealmContextHolder.get(),
    ...
    private static void applySubjectOverride(final JwtEncodingContext context, final Map<String, String> profile,
                                             final ClaimScopePublisher claimScopePublisher) {
        try {
            final String subjectClaim = claimScopePublisher.subjectForClient(io.helixiam.authorization.support.RealmScopedKey.pack(
                    io.helixiam.authorization.security.realm.RealmContextHolder.get(),
                    context.getRegisteredClient().getClientId()));
    ...
    private static void applyProtocolMappers(final JwtEncodingContext context, final Map<String, String> profile,
        ...
        try {
            final List<ProtocolMapperDto> mappers = clientMapperPublisher.forClient(io.helixiam.authorization.support.RealmScopedKey.pack(
                    io.helixiam.authorization.security.realm.RealmContextHolder.get(),
                    context.getRegisteredClient().getClientId()));
