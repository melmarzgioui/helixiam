...
class RealmExportServiceTest {
    ...
    void setUp() {
        ...
        service = new RealmExportService(realm, clients, saml, roles, scopes, idps, flows, orgs,
                ...
                mock(group.mfnr.authorization.amqp.group.GroupAdminPublisher.class),
                mock(group.mfnr.authorization.amqp.user.UserAdminPublisher.class),
    ...
    void export_includesPerClientSlices_keyedByOAuthClientId() {
        ...
        when(clientRolePub.listRoles(any())).thenReturn(List.of(new ClientRoleDto("r1", "gov", "web-app", "viewer",
                "Viewer")));
        when(clientRolePub.serviceAccountRoles(eq(group.mfnr.authorization.support.RealmScopedKey.pack("gov", "web-app"))))
                .thenReturn(List.of(new ServiceAccountRoleDto("s1", "gov", "web-app", "admin", "REALM", null)));
        when(resourcePub.allowedResourcesForClient(eq(group.mfnr.authorization.support.RealmScopedKey.pack("gov", "web-app"))))
